...
def main(pathIm):
    ...
    modelPath = '.\\trainedModel\\monModel.hdf5'
    #imagePath =  '.\\testImage\\girolle3.jpg'
    imagePath = pathIm
    imageSize = (50,50)
    label = ['amanite', 'cèpe', 'girolles']
...
def predict(modelPath,imagePath, imageSize, label):
    ...
    #on ecrit les prédictions dans un fichier
    f = open('Predictions.txt','w')
    f.write('---------- \n')
    f.write(' Prediction : \n')
    for i in range(0, len(label)):
       f.write('     ' + label[i] + ' : ' + "{0:.2f}%".format(prediction[0][i] * 100.)+'\n') 
    
    f.write('---------- \n')
    f.write('RESULTAT : ' + word + ' : ' + "{0:.2f}%".format(pred) +'\n')
    f.write('temps prediction : ' + "{0:.2f}secs".format(end-start)+'\n')
    f.write('---------- \n')
    f.close()
